package Simple is
   ...
   type Struct_With_Discriminant (D : Boolean := True) is record
      F1 : Integer := -1;
   ...
   type Nested_Struct is tagged record
      V1 : Simple_Struct;
      V2 : Simple_Struct;
      V3 : Struct_With_Discriminant (False);
      V4 : Struct_With_Discriminant;
   end record;

   type Nested_Struct_2 is new Nested_Struct with record
      Extra : Float := -1.0;
   end record;
   type Standard_Types is record
   ...
   type Standard_Types_Array is array (Natural range <>) of Standard_Types with
     Annotate => Auto_Io_Gen_Ignore;
