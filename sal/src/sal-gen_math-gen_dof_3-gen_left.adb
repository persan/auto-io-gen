...
package body SAL.Gen_Math.Gen_DOF_3.Gen_Left is
   ...
   is
      --  Ensure Angle < Pi. Find Angle_Axis. Apply [1] 2.3.1-3.
      use Math_Scalar;
   ...
   is
      --  [1] 2.3.1-7, followed by theta * n
      use Math_Scalar, Cart_Vector_Ops;
      Sin_Half : constant Real_Type := Mag (Cart_Vector_Type'(Quaternion.X, Quaternion.Y, Quaternion.Z));
   begin
      if Sin_Half > 0.0 then
         if Quaternion.S < 0.0 then
            return (-2.0 * Atan2 (Unchecked_Trig_Pair (Sin_Half, -Quaternion.S)) / Sin_Half) *
               Cart_Vector_Type'(Quaternion.X, Quaternion.Y, Quaternion.Z);
         else
            return (2.0 * Atan2 (Unchecked_Trig_Pair (Sin_Half, Quaternion.S)) / Sin_Half) *
               Cart_Vector_Type'(Quaternion.X, Quaternion.Y, Quaternion.Z);
         end if;
   ...
   is begin
      ...
      return ((X => 2.0 * (Quat.X * Quat.Y - Quat.S * Quat.Z),
   ...
   begin
      return
         (Right (X) + 2.0 * (Left.S * A (X) + Left.Y * A (Z) - Left.Z * A (Y)),
          Right (Y) + 2.0 * (Left.S * A (Y) + Left.Z * A (X) - Left.X * A (Z)),
          Right (Z) + 2.0 * (Left.S * A (Z) + Left.X * A (Y) - Left.Y * A (X)));
   end "*";
   ...
   is begin
      return Unchecked_Unit_Vector (Left * (Right (X), Right (Y), Right (Z)));
   end "*";
