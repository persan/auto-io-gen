--  Abstract:
--
--  Test Sal.Poly.Lists.Double, with various item types.
--
...
procedure Test_Poly_Lists_Double is
   Debug : Boolean := False;
begin
   ...
   Test_Storage_Pools.Set_Debug (Test_Poly_Lists_Double_Aux.Node_Storage_Pool, Debug);
   ...
   Test_Storage_Pools.Show_Storage (Test_Poly_Lists_Double_Aux.Node_Storage_Pool);
   Test_Storage_Pools.Check_Deallocated (Test_Poly_Lists_Double_Aux.Node_Storage_Pool);
   ...
   begin
      ...
      Replace (List, Iterator, 4);
      Print_List (List);
      if Current (Iterator) = 4 then
         Put_Line ("Next is 4");
      else
         Put_Line ("Error : Next is not 4");
      end if;
      Finalize (List);
   ...
   begin
      ...
      --  Full path coverage of sal.Poly.lists.double.Copy_Before, Insert
      ...
      ----------
      --  Full path coverage of sal.Poly.lists.double.Delete (list, iterator), Delete (private)
      ...
      ----------
      --  Full path coverage of sal.Poly.lists.double.Insert_After (Insert covered above)
      ...
      ----------
      --  Full path coverage of sal.Poly.lists.double.Insert_Before (Insert covered above)
      ...
      ----------
      --  Full path coverage of sal.Poly.lists.double.Splice_After
      ...
      --  path: source_first /= null, source_last /= null
      --  Source.head /= source_first, source.tail /= source_last
      --  Dest_tail /= null, Dest_After = null
      ...
      --  path: source_first /= null, source_last /= null
      --  Source.head /= source_first, source.tail /= source_last
      --  Dest.Tail /= null, Dest_After /= null, /= dest.tail
      ...
      --  path: source_first /= null, source_last /= null
      --  Source.head /= source_first, source.tail /= source_last
      --  Dest.Tail /= null, Dest_After /= null, dest_after = dest.tail
      Put_Line ("after tail");
      Finalize (List_1);
      Insert_Tail (List_1, 11);
      Insert_Tail (List_1, 12);
      ...
      ----------
      --  full path coverage of sal.Poly.lists.double.Splice_Before
      ...
      --  path: source_first /= null, source_last /= null
      --  Source.head /= source_first, source.tail /= source_last
      --  Dest_head /= null, Dest_Before = null
      Put_Line ("partial non-empty source, non-empty dest");
      Finalize (List_1);
      Insert_Tail (List_1, 11);
      Insert_Tail (List_1, 12);
      ...
      --  Source.head /= source_first, source.tail /= source_last
      --  Dest.head /= null, Dest_Before /= null, /= dest.head
      ...
      --  source_first /= null, source_last /= null
      --  Source.head /= source_first, source.tail /= source_last
      --  dest_head /= null, Dest_Before = dest.head
   ...
   ------------
   --  non-scalar item types
   --
   --  There are two differences in the body of SAL.Poly.lists.double
   --  for these types: Free_Item and Copy are not null. Free_Item is
   --  only called from Delete (private) and Replace; we need to do at
   --  least one delete and one replace. This mostly shows that you
   --  can instantiate more complex types.

   Put_Line ("Indefinite items (Symbols)");
   Symbol_Lists :
   declare
      use Test_Poly_Lists_Double_Aux.Symbols;
      use Lists;
   ...
   begin
      ...
      Put_Line ("Replacing First");
      Iterator := First (List);
      Replace (List, Iterator, (Discrete, -6, 6));
      Check_Print (List, "Symbols");
   ...
   Test_Storage_Pools.Check_Deallocated (Test_Poly_Lists_Double_Aux.Node_Storage_Pool);
   Test_Storage_Pools.Check_Deallocated (Test_Poly_Lists_Double_Aux.Symbols.Storage_Pool);
   ...
   Puppet_Lists :
   declare
      use Test_Poly_Lists_Double_Aux.Puppets;
      use Lists;
   ...
   begin
      ...
      Put_Line ("Replacing First");
      Iterator := First (List);
      Replace (List, Iterator, ("Beanie 5"));
      Check_Print (List, "Symbols");
   ...
   Test_Storage_Pools.Check_Deallocated (Test_Poly_Lists_Double_Aux.Node_Storage_Pool);
   Test_Storage_Pools.Check_Deallocated (Test_Poly_Lists_Double_Aux.Puppets.Storage_Pool);
end Test_Poly_Lists_Double;
