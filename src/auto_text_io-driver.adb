...
with Auto_Text_IO.Build;
with Auto_Text_IO.Generate;
with Auto_Text_IO.Options; use Auto_Text_IO.Options;
with Gnatvsn;
...
begin
   ...
   declare
      ...
      State          : Auto_Text_IO.Build.State_Type;
   ...
   begin
      ...
      then
         ...
         if not Quiet then
            Ada.Wide_Text_IO.Put ("auto_text_io: Compilation unit " & Unit_Full_Name (CU));
...
exception
   ...
   when Asis.Exceptions.ASIS_Failed =>
      New_Line;
      Put_Line ("auto_text_io: internal error. Run with -d option for more info.");
